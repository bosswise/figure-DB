...
<head>
  ...
  <style>
    /* 🚨 [PRO 솔루션] 유령 문구 완전 차단을 위한 화면 덮개 스타일 */
    html, body { 
      margin: 0; padding: 0; width: 100%; height: 100%; 
      overflow: hidden; /* 기존 스크롤 막음 (뒤에 있는 유령 문구 스크롤 방지) */
      background-color: #f7f3f0; 
    }

    /* GitHub 헤더 강제 숨김 시도 (2중 안전장치) */
    header, footer, .site-header, .gh-header, body > h1, body > h2 {
      display: none !important; opacity: 0 !important; pointer-events: none !important;
    }

    :root { 
      --primary: #fab005; --bg: #f7f3f0; --dark: #2d2926; --tag-gold: #ffeaa7; --modal-bg: rgba(0,0,0,0.98);
    }

    /* 🛡️ 박물관 전체를 감싸는 최상위 레이어 (유령 문구 위를 덮음) */
    #museum-wrapper {
      position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
      background-color: var(--bg);
      z-index: 2147483647; /* 브라우저 최대값 */
      overflow-y: auto; /* 내부에서 스크롤 되도록 변경 */
      font-family: 'Noto Sans KR', sans-serif;
    }

    /* 상단 레이아웃 (황금비율) */
    .main-title-area { 
      padding: 60px 0 40px; display: flex; align-items: center; justify-content: center; 
      max-width: 1500px; margin: 0 auto; gap: 50px; 
    }
    .hall-of-fame { 
      width: 300px; height: 400px; position: relative; cursor: pointer; flex-shrink: 0;
      border-radius: 35px; box-shadow: 0 20px 50px rgba(0,0,0,0.12); overflow: hidden; background: #fff; 
    }
    ...
    .header-mascot { 
      ...
      padding: 15px; box-shadow: 0 10px 40px rgba(0,0,0,0.06); margin-bottom: 20px; 
    }
    ...
    .total-stats-badge { 
      ...
      padding: 8px 22px; border-radius: 20px; font-size: 1rem; font-weight: 800; margin-top: 15px; 
    }
    ...
    .bookmark-container { 
      ...
      gap: 12px; padding: 0 25px 15px; transition: 0.6s ease; overflow: hidden; max-height: 1000px; 
    }
    ...
    /* 🖼️ 상세 모달 (마우스 추적 줌 & 라벨링) */
    .modal { display: none; position: fixed; inset: 0; background: var(--modal-bg); z-index: 2147483648; /* wrapper보다 높게 */ justify-content: center; align-items: center; padding: 40px; backdrop-filter: blur(30px); }
    ...
    .nav-btn { position: absolute; top: 50%; transform: translateY(-50%); width: 70px; height: 70px; background: rgba(255,255,255,0.98); border: none; border-radius: 50%; font-size: 2.5rem; cursor: pointer; z-index: 10; box-shadow: 0 10px 30px rgba(0,0,0,0.15); display: flex; align-items: center; justify-content: center; color: #333; transition: 0.3s; }
...
<script>
  ...
  async function init() {
    try {
      ...
      // 🚨 유령 문구 삭제 스크립트 (2차 방어선)
      setInterval(() => {
        document.body.childNodes.forEach(node => {
          // museum-wrapper, detailModal, script 태그가 아닌 것은 모두 제거
          if (node.nodeType === 1 && (node.id !== 'museum-wrapper' && node.id !== 'detailModal' && node.tagName !== 'SCRIPT')) {
            node.style.display = 'none';
          }
          // 텍스트 노드(figure-DB 같은 것) 제거
