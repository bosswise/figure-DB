...
<head>
  ...
  <style>
    ...
    .tag { font-size: 0.85rem; background: var(--tag-gold); color: #d35400; padding: 6px 14px; border-radius: 12px; font-weight: 800; }
    ...
    #modalImg { max-width: 90%; max-height: 90%; object-fit: contain; transition: transform 0.1s ease-out; }
    #modalImg.zoomed { cursor: zoom-out; transform: scale(3.5); }
    .nav-btn { position: absolute; top: 50%; transform: translateY(-50%); width: 70px; height: 70px; background: rgba(255,255,255,0.98); border: none; border-radius: 50%; font-size: 2.5rem; cursor: pointer; z-index: 10; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 30px rgba(0,0,0,0.15); }
    ...
    .info-label { font-size: 1rem; color: var(--primary); font-weight: 900; margin-bottom: 8px; }
    .info-value { font-size: 1.7rem; font-weight: 700; color: var(--dark); }
  </style>
</head>
<body>
...
<script>
  ...
  // 💡 [이름 결정 로직] M열(index 12) 우선, 없으면 B열(index 1)
  function getName(item) {
    return (item[12] && item[12].trim() !== "") ? item[12] : item[1];
  }
  ...
  function renderFilters() {
    ...
    for (const [cat, seriesSet] of Object.entries(menuMap)) {
      ...
      let html = `<span class="main-label" style="color:#eee; font-weight:800; min-width:120px;">${icon}${cat.toUpperCase()}</span><div class="sub-btns-scroll">`;
    ...
    grid.innerHTML = data.map((item, idx) => {
      ...
      return `<div class="card" data-series="${item[2]}" onclick="window.openModal(${allData.indexOf(item)})">
        ...
        <div class="content">
          ...
          <div class="tag-wrap"><span class="tag">#${item[10]}</span><span class="tag sec" style="background:#eee; color:#777;">#${item[2]}</span></div>
        </div>
      </div>`;
    }).join('');
  }
  ...
  window.openModal = function(idx) {
    ...
    document.getElementById('modalInfo').innerHTML = `
      <div class="info-item"><h2 style="font-size:3rem; font-weight:900; color:#2d2926; margin:0;">${name}</h2></div>
      ...
      <div class="info-item" style="border:none;"><span class="info-label">[ 특이사항 ]</span><p style="color:#666;">${item[9] || '내용 없음'}</p></div>
    `;
  ...
  window.handleZoomMove = function(e) { if (!isZoomed) return; const img = document.getElementById('modalImg'); const { left, top, width, height } = e.currentTarget.getBoundingClientRect(); const x = ((e.pageX - left - window.scrollX) / width) * 100; const y = ((e.pageY - top - window.scrollY) / height) * 100; img.style.transformOrigin = `${x}% ${y}%`; }
  window.updateModalImg = function() { const img = document.getElementById('modalImg'); img.src = `${imageBaseURL}${encodeURIComponent(currentImages[currentImgIdx])}.jpg`; isZoomed = false; img.classList.remove('zoomed'); }
