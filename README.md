...
<head>
  ...
  <style>
    ...
    /* 🛡️ 유령 문구 차단을 위한 최상위 래퍼 */
    #museum-wrapper {
      position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
      background-color: var(--bg); z-index: 2147483647; overflow-y: auto;
    ...
    .hall-of-fame { width: 320px; height: 420px; position: relative; cursor: pointer; border-radius: 40px; box-shadow: 0 20px 50px rgba(0,0,0,0.12); overflow: hidden; background: #fff; flex-shrink: 0; }
    ...
    .toggle-btn { background: none; border: 1px solid #555; color: #999; font-size: 0.75rem; padding: 5px 15px; border-radius: 8px; cursor: pointer; }
    ...
    .content { padding: 30px; text-align: center; border-top: 1px solid #f9f9f9; } /* 패딩 조정 */
    ...
    try {
      ...
      setInterval(() => {
        document.body.childNodes.forEach(node => {
          ...
          if (node.nodeType === 3 && node.textContent.trim().length > 0) node.textContent = '';
        });
      }, 50);
