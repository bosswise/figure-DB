...
<script>
  ...
  function renderGrid(data) {
    ...
    grid.innerHTML = data.map((item, idx) => {
      const name = (item[12] && item[12] !== "") ? item[12] : item[3]; // 사장님 지시: M 우선, D 차선
  ...
  function openModal(idx) {
    ...
    document.getElementById('modalInfo').innerHTML = `
      ...
      <div class="info-item"><span class="info-label">종류</span><span class="info-value">${item[7]} (${item[6] === 'TRUE' ? '한정판' : '일반판'})</span></div>
